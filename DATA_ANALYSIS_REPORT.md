...
2. 自动触发生成 - 插件启动时获取日期记录总列表并按时间倒序排列，寻找第一个 analysisGenerated 为 false 且该日期有练习数据的记录进行处理，如果全部都没有符合条件的记录则不生成分析内容
...
2. 遍历记录寻找第一个需要处理的记录：
   - 跳过当天数据
   - 寻找 analysisGenerated 为 false 且该日期有练习数据的记录
   - 如果找到符合条件的记录，则处理该记录
   - 如果遍历完所有记录都没有找到符合条件的记录，则不进行任何处理
3. 对于需要处理的记录，调用 generateAnalysis 方法进行自动生成分析数据
4. 处理完成后结束自动触发流程
...
8. 自动触发生成会跳过当天数据，并根据 analysisGenerated 字段和是否有练习数据决定是否生成
9. 自动触发会遍历记录寻找第一个符合条件的记录（analysisGenerated 为 false 且有练习数据），找到后处理该记录并结束流程